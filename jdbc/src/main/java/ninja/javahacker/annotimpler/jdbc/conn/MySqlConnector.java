package ninja.javahacker.annotimpler.jdbc.conn;
...
/// Immutable JDBC connector for MySQL databases.
///
/// @param host The database server hostname or IP address.
/// @param port The TCP port number on which the MySQL server listens.
...
) implements Connector.MandatoryAuthConnector<MySqlConnector>, Connector.HostConnector<MySqlConnector>
{
    ...
    /// The standard TCP port for MySQL (3306).
    ...
    /// Creates a `MySqlConnector` with the given connection parameters.
    ///
    /// @param host The database server hostname or IP address.
    /// @param port The TCP port number on which the MySQL server listens.
    /// @param user The database username.
    ...
    ///
    /// @return The standard `MySqlConnector` instance.
    @NonNull
    public static MySqlConnector std() {
        return STD;
    }

    /// Creates a `MySqlConnector` from optional field values, applying each present value over the defaults
    ...
    /// @return A new `MySqlConnector` with the applied overrides.
    ...
    /// @return A new `MySqlConnector` with the updated host.
    ...
    /// @return A new `MySqlConnector` with the updated user.
    ...
    /// @return A new `MySqlConnector` with the updated password.
    ...
    /// @return A new `MySqlConnector` with the updated database.
    ...
    /// @return A new `MySqlConnector` with the updated port.
    @NonNull
    @Override
